<template>
  <div>
    <div class="w-full bg-[#f7f7f7] rounded-xl px-12 py-8">
      <h1 class="font-bold text-3xl">Investor Edit</h1>

      <el-breadcrumb :separator-icon="ArrowRight">
        <el-breadcrumb-item :to="{ path: '/investors' }">Investors List</el-breadcrumb-item>
        <el-breadcrumb-item>Edit Investor </el-breadcrumb-item>
      </el-breadcrumb>
      ...
      <div class="mt-10">
        ...
        >
          ...
          <div class="flex justify-end mt-5 gap-4">
            ...
            <el-button type="primary" @click="submitForm(ruleFormRef)"> Edit </el-button>
          </div>
        </el-form>
      </div>
    </div>
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      try {
        const response = await apiClient.put(`/dashboard/station-investor/${route.params.id}/`, {
...
const getData = async () => {
  try {
    const response = await apiClient.get(`/dashboard/station-investor/${route.params.id}`)
    ruleForm.email = response.data.email
    ruleForm.username = response.data.username
    ruleForm.first_name = response.data.first_name
    ruleForm.last_name = response.data.last_name
    ruleForm.phone_number = response.data.phone_number
