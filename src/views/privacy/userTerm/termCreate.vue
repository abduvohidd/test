<template>
  <div>
    <h1 class="text-[24px]">{{ t('terms.termCreate') }}</h1>
      ...
      >
        <div class="flex w-full gap-4">
          <el-form-item :label="t('terms.termTable.title')" prop="title" class="w-[50%]">
            <el-input v-model="ruleForm.title" :placeholder="t('terms.termTable.title')" />
          </el-form-item>
          <el-form-item :label="t('terms.termTable.language')" prop="language" class="w-[50%]">
            <el-select v-model="ruleForm.language" :placeholder="t('terms.termTable.language')">
        ...
        <el-form-item :label="t('terms.termTable.content')" prop="content" class="mt-5">
          <!-- <Editor v-model="ruleForm.content" editorStyle="height: 320px"> </Editor> -->
          <el-input v-model="ruleForm.content" type="textarea" :placeholder="t('terms.termTable.content')" />
        </el-form-item>
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  await formEl.validate(async (valid, fields) => {
    if (valid) {
      try {
        const response = await apiClient.post(`/user/terms-and-conditions/`, {
        ...
        toast.error('terms and conditions with this language already exists.', {
          autoClose: 3000,
        })
      }
    } else {
